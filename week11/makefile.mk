# USAGE : cat design.csv | parallel --jobs 3 --colsep , --header : --eta --bar --verbose \ make -f makefile_2.mk process_sample SRR={SRR} SAMPLE={name} COVERAGE={coverage}
...
# Per-sample workflow (updated to include VCF annotation)
...
vcf_annotation:
	@echo "🧬 Annotating VCF files with snpEff..."
	@mkdir -p $(ANNOTATION_DIR)/$(SAMPLE)
	@if [ -f "$(VCF_DIR)/$(SAMPLE).vcf.gz" ]; then \
		...
		snpEff -c snpEff.config $(SNPEFF_DB) $(TEMP_DIR)/$(SAMPLE)_fixed.vcf > $(ANNOTATION_DIR)/$(SAMPLE)/$(SAMPLE)_annotated.vcf 2>&1; \
		if [ -f "snpEff_summary.html" ]; then mv snpEff_summary.html $(ANNOTATION_DIR)/$(SAMPLE)/$(SAMPLE)_snpEff_summary.html; fi; \
		if [ -f "snpEff_genes.txt" ]; then mv snpEff_genes.txt $(ANNOTATION_DIR)/$(SAMPLE)/$(SAMPLE)_snpEff_genes.txt; fi; \
		rm $(TEMP_DIR)/$(SAMPLE)_fixed.vcf; \
		echo "✅ Annotation completed for $(SAMPLE)"; \
		echo "   - Annotated VCF: $(ANNOTATION_DIR)/$(SAMPLE)/$(SAMPLE)_annotated.vcf"; \
		echo "   - Summary HTML: $(ANNOTATION_DIR)/$(SAMPLE)/$(SAMPLE)_snpEff_summary.html"; \
		echo "   - Genes TXT: $(ANNOTATION_DIR)/$(SAMPLE)/$(SAMPLE)_snpEff_genes.txt"; \
	else \
...
.PHONY: all genome index process_sample calculate_coverage download_reads fastqc align stats vcf vcf_annotation bigwig clean